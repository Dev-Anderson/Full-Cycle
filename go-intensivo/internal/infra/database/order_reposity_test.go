...
type OrderReposityTestSuite struct {
	suite.Suite
	Db *sql.DB
}

func (suite *OrderReposityTestSuite) SetupSuite() {
...
func (suite *OrderReposityTestSuite) TearDownSuite() {
	suite.Db.Close()
}

func (suite *OrderReposityTestSuite) TestSavingOrder() {
	order, err := entity.NewOrder("123", 10.0, 1.0)
	suite.NoError(err)
	suite.NoError(order.CalculateFinalPrice())
	repo := NewOrderReposityr(suite.Db)
	err = repo.Save(order)
	suite.NoError(err)
